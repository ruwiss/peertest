						...
						<tr class="border-t border-zinc-100 dark:border-zinc-800">
							...
							<td class="p-3">
								<div class="flex items-center gap-1.5">
									<!-- Sik kullanilan puan ayari gorunur; yikici aksiyonlar ⋯ menusunde. -->
									<form
										method="POST"
										action="?/adjustScore"
										use:enhance
									...
									<ActionMenu>
										{#snippet children(close)}
											<form method="POST" action="?/shadowban" use:enhance onsubmit={close}>
												<input type="hidden" name="userId" value={u.id} />
												<button type="submit" class={menuItemCls}
													>{m.admin_action_shadowban()}</button
												>
											</form>
											...
											<form
												...
												use:enhance
												onsubmit={(e) => {
													if (!confirm(m.admin_action_delete_confirm())) {
														e.preventDefault();
														return;
													}
													close();
	...
	{:else}
		<div class="space-y-2">
			{#each data.frozen as f (f.id)}
				<div
					class="flex items-center justify-between rounded-xl border border-zinc-200/80 bg-white p-3 dark:border-zinc-800 dark:bg-zinc-900"
				>
					<div>
