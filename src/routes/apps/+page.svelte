<script lang="ts">
	...
	function navigate(nextQ: string, nextPage: number) {
		...
		goto(qs ? `/apps?${qs}` : '/apps', { replaceState: false, keepFocus: true, noScroll: false });
	}
...
<section class="mx-auto max-w-4xl px-4 py-8 sm:py-12">
	<!-- Baslik satiri: modern bold baslik + sag taraf My Apps linki -->
	<div class="mb-6 flex flex-wrap items-end justify-between gap-3">
		...
		{#if data.loggedIn}
			<a
				href="/my-apps"
				class="text-sm font-medium text-zinc-500 transition-colors hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-zinc-100"
			>
				{m.home_my_apps()}
			</a>
		{/if}
	...
	<div class="relative mb-7">
		...
		<input
			...
			class="w-full rounded-xl border border-zinc-200 bg-white py-3 pr-10 pl-11 text-[15px] text-zinc-900 placeholder:text-zinc-400 focus:border-tg-500 focus:outline-none dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-100"
		/>
		{#if query}
			<button
				type="button"
	...
	{:else}
		<div
			class="divide-y divide-zinc-100 overflow-hidden rounded-2xl border border-zinc-200/70 bg-white shadow-soft dark:divide-zinc-800/80 dark:border-zinc-800 dark:bg-zinc-900 dark:shadow-none"
		>
		...
		{#if data.totalPages > 1}
			<nav class="mt-6 flex items-center justify-between gap-2 text-sm">
				<button
					...
					class="inline-flex items-center gap-1.5 rounded-full border border-zinc-200 px-4 py-1.5 font-medium text-zinc-700 hover:bg-zinc-50 disabled:cursor-not-allowed disabled:opacity-40 dark:border-zinc-700 dark:text-zinc-300 dark:hover:bg-zinc-800"
				>
					<svg
						class="size-4"
						viewBox="0 0 20 20"
