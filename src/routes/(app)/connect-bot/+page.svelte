...
<section class="mx-auto max-w-lg px-4 py-12 sm:py-16">
	<div class="mb-6 text-center">
		<div
			class="mx-auto mb-4 inline-flex size-14 items-center justify-center rounded-2xl bg-tg-50 text-tg-700 dark:bg-tg-500/15 dark:text-tg-300"
		>
	...
	{#if data.botStarted}
		...
		>
			<span
				class="inline-flex size-8 shrink-0 items-center justify-center rounded-full bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-300"
			>
				<Icon name="check-circle" class="size-5" />
			</span>
			<div class="min-w-0 flex-1">
				<p class="text-sm font-medium text-emerald-800 dark:text-emerald-300">
					{m.connect_bot_connected()}
				</p>
				{#if data.redirectTo && data.redirectTo !== '/'}
					<a
						href={data.redirectTo}
						class="mt-1 inline-block text-xs font-medium text-emerald-700 hover:underline dark:text-emerald-400"
					>
	...
	{:else}
		<!-- Zorunluluk uyarisi -->
		<div
			class="mb-4 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-200"
		>
		...
		<div
			class="space-y-5 rounded-2xl border border-zinc-200/70 bg-white p-6 shadow-soft dark:border-zinc-800 dark:bg-zinc-900 dark:shadow-none"
		>
			<ol class="space-y-3 text-sm text-zinc-700 dark:text-zinc-300">
				<li class="flex items-start gap-3">
					<span
						class="inline-flex size-6 shrink-0 items-center justify-center rounded-full bg-tg-100 font-mono text-xs font-semibold text-tg-700 dark:bg-tg-500/20 dark:text-tg-300"
						>1</span
					>
